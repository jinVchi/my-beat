...
export default class Enemy extends Phaser.GameObjects.Container {
    ...
    constructor(scene: Phaser.Scene, x: number, y: number, maxHealth = 100) {
        ...
        this.bodyRect = scene.add.rectangle(0, 0, BODY_WIDTH, BODY_HEIGHT, 0xcc3333);
        this.bodyRect.setStrokeStyle(2, 0xffffff);
        this.add(this.bodyRect);

        this.label = scene.add.text(0, -BODY_HEIGHT / 2 - 14, 'ENEMY', {
            fontSize: '12px',
            color: '#ffffff',
            fontFamily: 'Arial',
        }).setOrigin(0.5, 0.5);
        ...
        this.healthBarFill = scene.add.rectangle(0, barY, barWidth - 2, barHeight - 2, 0x00cc00);
